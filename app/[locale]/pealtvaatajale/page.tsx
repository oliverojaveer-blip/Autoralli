...
export async function generateMetadata({ params }: LocaleParams): Promise<Metadata> {
  ...
  return pageMetadata(locale, '/pealtvaatajale', getDictionary(locale).pages.spectators.title)
}

export default async function PealtvaatajalePage({ params }: LocaleParams) {
  const locale = toLocale((await params).locale)
  const t = getDictionary(locale)
  const page = t.pages.spectators
