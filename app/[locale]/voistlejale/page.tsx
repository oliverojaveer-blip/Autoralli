...
export async function generateMetadata({ params }: LocaleParams): Promise<Metadata> {
  ...
  return pageMetadata(locale, '/voistlejale', getDictionary(locale).pages.competitors.title)
}

export default async function VoistlejalePage({ params }: LocaleParams) {
  const locale = toLocale((await params).locale)
  const t = getDictionary(locale)
  const page = t.pages.competitors
